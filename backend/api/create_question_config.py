...
def main():
    try:
        latest_question = read_approved_question('data/approved_question.json')
        if not latest_question:
            raise ValueError("No question found in approved_question.json")
        
        question_config = generate_question_config(latest_question)
        if save_json(question_config, 'data/question_config.json'):
            print_first_10_lines('data/question_config.json')
    except Exception as e:
